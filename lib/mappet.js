...
function mappet(schema, options) {
    ...
    var flat = flatten(schema);
    var prepared = Object.keys(flat)
        .map(function (path) { return typeof flat[path] === "string" ? [path, flat[path]] : [path].concat(flat[path]); });
    return function (source) {
        var base = greedyMode === true ? clone(source) : {};
        return prepared
            .map(function (_a) {
